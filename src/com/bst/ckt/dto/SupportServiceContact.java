package com.bst.ckt.dto;

public class SupportServiceContact {
	private int serviceContctId;
	private String subject;
	private String serviceContactType;
	private String relatedTo;
	private String assignedTo;
	private String priority;
	private String status;
	private String trackingUnit;
	private String usedUnits;
	private String totalUnits;
	private String startDate;
	private String dueDate;
	private String contactName;
	private int createdBy;
	private String createdDate;
	private int searchId;
	private String searchValue;

	/**
	 * @return the serviceContctId
	 */
	public int getServiceContctId() {
		return serviceContctId;
	}

	/**
	 * @param serviceContctId
	 *            the serviceContctId to set
	 */
	public void setServiceContctId(int serviceContctId) {
		this.serviceContctId = serviceContctId;
	}

	/**
	 * @return the subject
	 */
	public String getSubject() {
		return subject;
	}

	/**
	 * @param subject
	 *            the subject to set
	 */
	public void setSubject(String subject) {
		this.subject = subject;
	}

	/**
	 * @return the serviceContactType
	 */
	public String getServiceContactType() {
		return serviceContactType;
	}

	/**
	 * @param serviceContactType
	 *            the serviceContactType to set
	 */
	public void setServiceContactType(String serviceContactType) {
		this.serviceContactType = serviceContactType;
	}

	/**
	 * @return the relatedTo
	 */
	public String getRelatedTo() {
		return relatedTo;
	}

	/**
	 * @param relatedTo
	 *            the relatedTo to set
	 */
	public void setRelatedTo(String relatedTo) {
		this.relatedTo = relatedTo;
	}

	/**
	 * @return the assignedTo
	 */
	public String getAssignedTo() {
		return assignedTo;
	}

	/**
	 * @param assignedTo
	 *            the assignedTo to set
	 */
	public void setAssignedTo(String assignedTo) {
		this.assignedTo = assignedTo;
	}

	/**
	 * @return the priority
	 */
	public String getPriority() {
		return priority;
	}

	/**
	 * @param priority
	 *            the priority to set
	 */
	public void setPriority(String priority) {
		this.priority = priority;
	}

	/**
	 * @return the status
	 */
	public String getStatus() {
		return status;
	}

	/**
	 * @param status
	 *            the status to set
	 */
	public void setStatus(String status) {
		this.status = status;
	}

	/**
	 * @return the trackingUnit
	 */
	public String getTrackingUnit() {
		return trackingUnit;
	}

	/**
	 * @param trackingUnit
	 *            the trackingUnit to set
	 */
	public void setTrackingUnit(String trackingUnit) {
		this.trackingUnit = trackingUnit;
	}

	/**
	 * @return the usedUnits
	 */
	public String getUsedUnits() {
		return usedUnits;
	}

	/**
	 * @param usedUnits
	 *            the usedUnits to set
	 */
	public void setUsedUnits(String usedUnits) {
		this.usedUnits = usedUnits;
	}

	/**
	 * @return the totalUnits
	 */
	public String getTotalUnits() {
		return totalUnits;
	}

	/**
	 * @param totalUnits
	 *            the totalUnits to set
	 */
	public void setTotalUnits(String totalUnits) {
		this.totalUnits = totalUnits;
	}

	/**
	 * @return the startDate
	 */
	public String getStartDate() {
		return startDate;
	}

	/**
	 * @param startDate
	 *            the startDate to set
	 */
	public void setStartDate(String startDate) {
		this.startDate = startDate;
	}

	/**
	 * @return the dueDate
	 */
	public String getDueDate() {
		return dueDate;
	}

	/**
	 * @param dueDate
	 *            the dueDate to set
	 */
	public void setDueDate(String dueDate) {
		this.dueDate = dueDate;
	}

	/**
	 * @return the contactName
	 */
	public String getContactName() {
		return contactName;
	}

	/**
	 * @param contactName
	 *            the contactName to set
	 */
	public void setContactName(String contactName) {
		this.contactName = contactName;
	}

	/**
	 * @return the createdBy
	 */
	public int getCreatedBy() {
		return createdBy;
	}

	/**
	 * @param createdBy
	 *            the createdBy to set
	 */
	public void setCreatedBy(int createdBy) {
		this.createdBy = createdBy;
	}

	/**
	 * @return the createdDate
	 */
	public String getCreatedDate() {
		return createdDate;
	}

	/**
	 * @param createdDate
	 *            the createdDate to set
	 */
	public void setCreatedDate(String createdDate) {
		this.createdDate = createdDate;
	}

	/**
	 * @return the searchId
	 */
	public int getSearchId() {
		return searchId;
	}

	/**
	 * @param searchId
	 *            the searchId to set
	 */
	public void setSearchId(int searchId) {
		this.searchId = searchId;
	}

	/**
	 * @return the searchValue
	 */
	public String getSearchValue() {
		return searchValue;
	}

	/**
	 * @param searchValue
	 *            the searchValue to set
	 */
	public void setSearchValue(String searchValue) {
		this.searchValue = searchValue;
	}

}
